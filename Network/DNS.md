...
> + 이때 **naver.com이라는 것은 도메인 이름**으로 네이버가 제공하는 서비스 서버들의 대표 이름으로 볼 수 있다.
> + 또한 네이버 메일 mail.naver.com 이나 네이버 지식인 kin.naver.com 처럼 서브 도메인에 위치하는 mail과 kin은 호스트 이름으로 유추해볼 수도 있겠지만
> + 서브 도메인에는 실제 호스트 이름을 직접 사용하기 보다는 **서비스명(mail, news, kin 등)이나 지역명(Korea, America 등)을 활용한 이름**을 사용한다.
