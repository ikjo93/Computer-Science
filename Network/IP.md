...
- 순서 보장
- 신뢰할 수 있는 프로토콜
- 현대는 대부분 TCP를 사용하나 최근에는 UDP가 새롭게 부상하는 중
...
- 애플리케이션 계층
  - 
  - 웹 브라우저, 게임, 채팅 프로그램 등의 프로세스들은 **소켓(SOCKET) 라이브러리**를 창구로 사용하여 데이터를 주고 받는다.
  - 소켓은 **IP 주소와 포트(PORT) 번호**로 정의되며 **TCP(Transmission Control Protocol)와 UDP(User Datagram Protocol)가** 있다.
  - 클라이언트의 요청이 있을 때만 서버가 응답하여 해당 정보를 전송하고 곧바로 연결을 종료하는 HTTP 통신과 달리 소켓 통신은 **클라이언트와 서버가 특정 포트를 통해 실시간으로, 양방향으로 통신**한다.
- 
- 
